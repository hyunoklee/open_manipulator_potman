...
void OpenManipulatorMaster::setModeState(char ch)
{
  ...
  else if(ch == '3')
  {
    ...
    if( record_buffer_.size() > 0 ){
	    std::ofstream outFile("/home/hyunoklee/output.txt");
	    for(int i = 0 ; i < record_buffer_.size() ; i++){
		        outFile << record_buffer_.at(i).joint_angle[0]<<','<< record_buffer_.at(i).joint_angle[1]<<','\
				<< record_buffer_.at(i).joint_angle[2]<<','<< record_buffer_.at(i).joint_angle[3]<<','\
				<< record_buffer_.at(i).tool_position<< std::endl;
		}
	    outFile.close();
    }
  }
